...
randIndex = [1:n;2:n,1];
randIndex = randIndex(:,randperm(n,2));
...
for counter = 1:1
    ...
    while(endFlag ~=1)
        ...
        if ~(base(1) == ccwList(end) || base(2) == ccwList(end))
            compareCenter2 = sum([polygon(base,:); polygon(cwList(j),:)])/3;
            compareCenter1 = sum([polygon(base,:); polygon(ccwList(i),:)])/3;
            dist1 = norm(cross([compareCenter1 - startCenter,0],[startEndVector,0]))/norm(startEndVector);
            dist2 = norm(cross([compareCenter2 - startCenter,0],[startEndVector,0]))/norm(startEndVector);
            if dist1>dist2
        ...
        % saveData{itr} = [ccwList(i),cwList(j)];
...
toc
set.minPathPerm = 1:length(set.triangles);
plotTriangulationSet(set,polygon,[startIndex;endIndex]);