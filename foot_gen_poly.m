function T = foot_gen_poly(T,L_nom,Ground)
...
if T.Parent_Index == 0
    ...
    p3 = [1 2 3];
    p3([p1 p2]) = [];
    T.foot(p3*3-2:p3*3,:) = foot_cal_pol(T,L_nom,p1,p2,p3,Ground);
    ...
    if T.face == 3
        ...
        %     p3 = [1 2 3];
        %     p3([p1 p2]) = [];
        %     T.foot(p3*2-1:p3*2,:) = foot_cal_pol(T,L_nom,p1,p2,p3,Ground);
...
n = T.n;
penta_angle = pi()*3/5;
penta_const = 2*L_nom*cos((pi() - penta_angle)/2);
...
[temp,orientation] = isPointInPolygon([x1 y1 z1;x2 y2 z2;x3 y3 z3],[x1+x2+x3 y1+y2+y3 z1+z2+z3]./3);

h = sqrt(abs(penta_const^2 - ( norm([x2-x1 y2-y1])/2)^2) );
a = y2 - y1;
b = x1 - x2;
c = (x2 - x1)*y1 + (y1 - y2)*x1;
...
footVectors1 = [foot_x-x1 foot_y-y1 foot_z-z1]/norm([foot_x-x1 foot_y-y1 foot_z-z1]);
footVectors2 = [foot_x-x2 foot_y-y2 foot_z-z2]/norm([foot_x-x2 foot_y-y2 foot_z-z2]);

if orientation == 1
    foot_r = [x1 y1 z1] + ([cos(-(pi()-penta_angle)/2) -sin(-(pi()-penta_angle)/2) 0;sin(-(pi()-penta_angle)/2) cos(-(pi()-penta_angle)/2) 0;0 0 1]*footVectors1'*L_nom)';
    foot_l = [x2 y2 z2] + ([cos((pi()-penta_angle)/2) -sin((pi()-penta_angle)/2) 0;sin((pi()-penta_angle)/2) cos((pi()-penta_angle)/2) 0;0 0 1]*footVectors2'*L_nom)';
elseif orientation == -1
    foot_r = [x2 y2 z2] + ([cos(-(pi()-penta_angle)/2) -sin(-(pi()-penta_angle)/2) 0;sin(-(pi()-penta_angle)/2) cos(-(pi()-penta_angle)/2) 0;0 0 1]*footVectors2'*L_nom)';
    foot_l = [x1 y1 z1] + ([cos((pi()-penta_angle)/2) -sin((pi()-penta_angle)/2) 0;sin((pi()-penta_angle)/2) cos((pi()-penta_angle)/2) 0;0 0 1]*footVectors1'*L_nom)';
    % foot_l = [x2 y2 z2] + ([cos(-(pi()-penta_angle)/2) -sin(-(pi()-penta_angle)/2) 0;sin(-(pi()-penta_angle)/2) cos(-(pi()-penta_angle)/2) 0;0 0 1]*footVectors2'*L_nom)';
    % foot_r = [x1 y1 z1] + ([cos((pi()-penta_angle)/2) -sin((pi()-penta_angle)/2) 0;sin((pi()-penta_angle)/2) cos((pi()-penta_angle)/2) 0;0 0 1]*footVectors1'*L_nom)';
end


foot_init = [foot_l;foot_x foot_y foot_z;foot_r];
